    ...
    <div className="flex-1 p-margin-page bg-background h-full overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-stack-md mb-stack-lg">
          <div>
            <h2 className="font-display-lg text-display-lg text-on-surface">Audit History</h2>
            <p className="font-body-lg text-body-lg text-on-surface-variant">Trace every AI decision and human intervention across your project lifecycle.</p>
          </div>
          <div className="flex items-center gap-stack-sm">
            <div className="flex bg-surface border border-outline-variant rounded-lg p-1">
              <button className={getBtnClass(filter === 'all')} onClick={() => setFilter('all')}>All Time</button>
              <button className={getBtnClass(filter === '7d')} onClick={() => setFilter('7d')}>Last 7d</button>
              <button className={getBtnClass(filter === 'custom')} onClick={() => alert("Tính năng chọn ngày đang được phát triển!")}>Custom</button>
            </div>
          </div>
        </div>
        ...
        <div className="relative pl-12 space-y-stack-lg min-h-[500px]">
          ...
          {loading ? (
            <div className="py-8 text-center text-on-surface-variant">Đang tải lịch sử...</div>
          ) : filteredHistory.length === 0 ? (
            <div className="py-8 text-center text-on-surface-variant relative z-10">Chưa có lịch sử hoạt động.</div>
          ) : (
                  ...
                  <div className="relative group mb-stack-lg">
                    ...
                    >
                      <div className="flex justify-between items-start mb-stack-sm">
                        <div>
                          <span className="font-label-md text-label-md text-secondary uppercase tracking-wider">AI AGENT ACTION</span>
                          <h3 className="font-headline-sm text-headline-sm mt-1">{item.jira_key ? `Generated ${item.jira_key}` : 'Generated Backlog Item'}</h3>
                        </div>
                        <div className="text-right">
                          <p className="font-mono-sm text-mono-sm text-on-surface-variant">{date.toLocaleTimeString()}</p>
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-green-100 text-green-700 font-label-md text-[10px] uppercase">Automated</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-stack-md">
                        ...
                        <span className="font-label-md text-on-surface-variant">Type: {item.story?.story_type || 'Unknown'}</span>
